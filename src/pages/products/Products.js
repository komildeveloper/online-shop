...
import phoneBanner from '../../assets/img/13pro.png'
  ...
  function selectButtonPhone(e) {
    ...
    if (phones[index].selectBool) {
      fetch(`http://localhost:3001/selected`, {
        ...
        body: JSON.stringify(phones[index])
      })
    } else {
  ...
  function selectButtonNotebook(e) {
    ...
    if (notebooks[indexNote].selectBool) {
      fetch(`http://localhost:3001/selected`, {
        ...
        body: JSON.stringify(notebooks[indexNote])
      })
    } else {
      fetch(`http://localhost:3001/selected/${id}`, {
        method: 'Delete'
  ...
  function selectButtonAccessorie(e) {
    ...
    if (accessories[indexAcc].selectBool) {
      fetch(`http://localhost:3001/selected`, {
        ...
        body: JSON.stringify(accessories[indexAcc])
      })
    } else {
      fetch(`http://localhost:3001/selected/${id}`, {
        method: 'Delete'
          ...
          return (
            ...
            >
              ...
              <p id={phone.id} className="product-price">
                {phone?.price} {dataProduct[lang]?.content3}
              </p>
            </NavLink>
          )
        })}
        ...
        {notebooks.map(notebook => {
          ...
          return (
            <NavLink
              ...
              className="card"
              to="/notebook"
              onClick={e => setNotebookId(e.target.id)}
            >
              <button
                id={notebook.id}
                onClick={e => selectButtonNotebook(e)}
                className="card-like btn"
              >
                <img
                  id={notebook.id}
              ...
              <img
                id={notebook.id}
                className=" mb-4"
                src={notebook?.imgLinks[0]}
                alt="photo"
                width="240"
                height="230"
              />
              <div id={notebook.id} className="card-bottom">
                <p id={notebook.id} className="product-name">
                  {notebook?.name}
                </p>
                <p id={notebook.id} className="raiting">
                  <img
                    id={notebook.id}
                    className=" star-icon"
                    src={starIcon}
              ...
              <p id={notebook.id} className="product-price">
                {notebook?.price} {dataProduct[lang]?.content3}
              </p>
            </NavLink>
          )
        })}
        ...
        {accessories.map(accessorie => {
          ...
          return (
            <NavLink
              ...
              className="card"
              to="/accessorie"
              onClick={e => setAccessorieId(e.target.id)}
            >
              <button
                onClick={e => selectButtonAccessorie(e)}
                id={accessorie.id}
                className="card-like btn"
              >
                <img
              ...
              <img
                id={accessorie.id}
                className=" mb-4"
                src={accessorie?.imgLinks[0]}
                alt="photo"
                width="240"
                height="230"
              />
              <div id={accessorie.id} className="card-bottom">
                <p id={accessorie.id} className="product-name">
                  {accessorie?.name}
                </p>
                <p id={accessorie.id} className="raiting">
                  <img
                    id={accessorie.id}
                    className=" star-icon"
                    src={starIcon}
              ...
              <p id={accessorie.id} className="product-price">
                {accessorie?.price} {dataProduct[lang]?.content3}
              </p>
            </NavLink>
          )
        })}
